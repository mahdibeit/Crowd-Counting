...
class SASNet(nn.Module):
    def __init__(self, pretrained=True, args=None):
        ...
        self.Down= nn.AvgPool2d(8,stride=8)
    ...
    def forward(self, x):
        ...
        x5_density = F.upsample_nearest(x5_density, size=matchsize)
        x4_density = F.upsample_nearest(x4_density, size=matchsize)
        x3_density = F.upsample_nearest(x3_density, size=matchsize)
        x2_density = F.upsample_nearest(x2_density, size=matchsize)
        x1_density = F.upsample_nearest(x1_density, size=matchsize)
